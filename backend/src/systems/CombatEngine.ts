...
export class CombatEngine {
  ...
  static simulateRound(attackerFleet: Fleet, defenderFleet: Fleet) {
    ...
    // Use import instead of require for BuffDebuffEngine
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    // import { BuffDebuffEngine } from './BuffDebuffEngine';
    // For dynamic assignment, use a type-safe approach
    type BuffDebuffEngineType = { tick: () => void; buffs: any[]; debuffs: any[] };
    if (!(attackerFleet as { _buffEngine?: BuffDebuffEngineType })._buffEngine) {
      (attackerFleet as { _buffEngine?: BuffDebuffEngineType })._buffEngine = new (await import('./BuffDebuffEngine')).BuffDebuffEngine();
    }
    if (!(defenderFleet as { _buffEngine?: BuffDebuffEngineType })._buffEngine) {
      (defenderFleet as { _buffEngine?: BuffDebuffEngineType })._buffEngine = new (await import('./BuffDebuffEngine')).BuffDebuffEngine();
    }
    const attackerBuffs = (attackerFleet as { _buffEngine: BuffDebuffEngineType })._buffEngine;
    const defenderBuffs = (defenderFleet as { _buffEngine: BuffDebuffEngineType })._buffEngine;
    ...
    function logBuffs(fleet: Fleet, label: string, buffs: any) {
      if (buffs.buffs.length > 0) log.push(`${label} buffs: ` + buffs.buffs.map((b: { type: string; stacks: number }) => `${b.type} x${b.stacks}`).join(', '));
      if (buffs.debuffs.length > 0) log.push(`${label} debuffs: ` + buffs.debuffs.map((d: { type: string; stacks: number }) => `${d.type} x${d.stacks}`).join(', '));
    }
