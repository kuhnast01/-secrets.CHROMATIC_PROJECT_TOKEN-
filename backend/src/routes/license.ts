...
} from '../security/licenseRevocations';
...
function getActorMetadata(req: Record<string, unknown>) {
  const user = (req.user as Record<string, unknown> | undefined) ?? undefined;
  return {
...
function createAuditMetadata(req: Record<string, unknown>) {
  return {
...
function parseBodyLicenseKey(body: unknown): string | undefined {
  if (!body || typeof body !== 'object') {
    return undefined;
  }

  const candidate = (body as Record<string, unknown>).licenseKey;
...
function parseNowMs(body: unknown): number | undefined {
  if (!body || typeof body !== 'object') {
    return undefined;
  }

  const candidate = (body as Record<string, unknown>).nowMs;
...
function parseOptionalNumber(body: unknown, key: string): number | undefined {
  if (!body || typeof body !== 'object') {
    return undefined;
  }
  const value = (body as Record<string, unknown>)[key];
...
function parseOptionalBoolean(body: unknown, key: string): boolean | undefined {
  if (!body || typeof body !== 'object') {
    return undefined;
  }
  const value = (body as Record<string, unknown>)[key];
...
function parseOptionalStringArray(body: unknown, key: string): string[] | undefined {
  if (!body || typeof body !== 'object') {
    return undefined;
  }
...
router.get('/status', authenticateJWT, authorizeRoles('admin', 'auditor'), (req, res) => {
  const mode = getLicenseMode();
  const revokedHashes = getRuntimeRevokedHashes();
  const validation = validateLicenseKey(
    resolveLicenseKey(req.headers['x-poseidon-license']),
  ...
  res.status(200).json({
    ...
    exemptPaths: getExemptPaths(),
  });
});

router.get('/revocations', authenticateJWT, authorizeRoles('admin', 'auditor'), (req, res) => {
  const entries = listRuntimeRevocations();
  res.status(200).json({
...
router.post('/revocations', authenticateJWT, authorizeRoles('admin'), (req, res) => {
  ...
  if (!keyHash && licenseKey) {
    keyHash = hashLicenseKey(normalizeLicenseKey(licenseKey));
  }
  ...
  try {
    const actor = getActorMetadata(req as unknown as Record<string, unknown>);
    const entry = addRuntimeRevocation(keyHash, reason, actor.actorId || undefined);
...
router.delete('/revocations/:keyHash', authenticateJWT, authorizeRoles('admin'), (req, res) => {
  const removed = removeRuntimeRevocation(req.params.keyHash);
  if (!removed) {
    return res.status(404).json({ error: 'revocation_not_found' });
  }

  const audit = createAuditMetadata(req as unknown as Record<string, unknown>);
  logger.info(
    {
...
router.post('/tools/hash', authenticateJWT, authorizeRoles('admin'), (req, res) => {
  const licenseKey = parseBodyLicenseKey(req.body);
  if (!licenseKey) {
    logger.warn(
      {
        ...
        path: req.path,
        method: req.method,
        ...getActorMetadata(req as unknown as Record<string, unknown>),
      },
...
router.post('/tools/validate', authenticateJWT, authorizeRoles('admin'), (req, res) => {
  const licenseKey = parseBodyLicenseKey(req.body);
  if (!licenseKey) {
    logger.warn(
      {
...
router.post('/tools/rotate', authenticateJWT, authorizeRoles('admin'), (req, res) => {
  ...
  const revokedHashes = getRuntimeRevokedHashes();
  const sourceValidation = validateLicenseKey(sourceLicenseKey, licenseSecret, revokedHashes);
  ...
  const claims = {
    ...
    rotatedFrom: hashLicenseKey(normalizeLicenseKey(sourceLicenseKey)),
  };

  const rotatedLicenseKey = createLicenseKey(claims, licenseSecret);
  const oldKeyHash = hashLicenseKey(normalizeLicenseKey(sourceLicenseKey));
  const newKeyHash = hashLicenseKey(normalizeLicenseKey(rotatedLicenseKey));
  const audit = createAuditMetadata(req as unknown as Record<string, unknown>);

  if (revokePrevious) {
    addRuntimeRevocation(oldKeyHash, 'rotated_license', audit.actorId || undefined);
  }
