...
describe('license security helpers', () => {
  it('defaults to warn mode for invalid values', () => {
    expect(getLicenseMode({ POSEIDON_LICENSE_ENFORCEMENT: 'invalid' } as NodeJS.ProcessEnv)).toBe('warn');
  });
  ...
  it('parses revoked hash list from env', () => {
    const values = getRevokedLicenseHashes({
      POSEIDON_LICENSE_REVOKED_HASHES:
        '  aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa , BB,cc,deadbeef,FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF  ',
  ...
  it('rejects revoked valid keys before signature/claims checks complete', () => {
    ...
    const revoked = new Set<string>([hashLicenseKey(normalizeLicenseKey(key))]);
