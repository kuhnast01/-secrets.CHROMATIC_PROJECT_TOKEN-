...
import { LiveOpsValidator, LiveOpsConfig } from './liveops-validator';

describe('LiveOpsValidator', () => {
  let validator: LiveOpsValidator;
  ...
  it('should validate event config with id', () => {
    const config: LiveOpsConfig = { type: 'event', data: { id: 'evt1' } };
  ...
  it('should invalidate event config without id', () => {
    const config: LiveOpsConfig = { type: 'event', data: {} };
    const result = validator.validate(config);
  ...
  it('should simulate event', () => {
    const config: LiveOpsConfig = { type: 'event', data: { id: 'evt2' } };
