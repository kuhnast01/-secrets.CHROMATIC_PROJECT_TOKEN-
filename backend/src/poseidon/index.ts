...
export async function createPoseidonApp() {
  ...
  let poseidonSearch: PoseidonSearch | null = null;
  ...
  app.post('/poseidon/query', async (req, res) => {
    ...
    let logData: any = { query, fileContent, filePath, error, type };
    if (filePath) {
      try {
  ...
  app.post('/poseidon/search_code', async (req, res) => {
    if (!poseidonSearch) return res.status(503).json({ error: 'Semantic search not initialized.' });
