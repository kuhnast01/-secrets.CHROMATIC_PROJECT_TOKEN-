...
export async function generate3DAsset(prompt, type) {
    const url = process.env.GEN3D_API_URL;
    const key = process.env.GEN3D_API_KEY;
    if (!url || !key)
        throw new Error('GEN3D API URL or KEY not set');
    const res = await axios.post(url, { prompt, type }, { headers: { 'Authorization': `Bearer ${key}` } });
    return res.data;
}
...
export async function synthesizeVoice(text) {
    ...
    const res = await axios.post(url, { text }, { headers: { 'xi-api-key': key } });
    return res.data;
}
