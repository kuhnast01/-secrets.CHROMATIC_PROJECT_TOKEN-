...
import { createLicenseKey, getExemptPaths, getLicenseMode, hashLicenseKey, normalizeLicenseKey, resolveLicenseKey, validateLicenseKey, } from '../security/license';
import { addRuntimeRevocation, getRuntimeRevokedHashes, listRuntimeRevocations, removeRuntimeRevocation, } from '../security/licenseRevocations';
const router = Router();
...
router.get('/status', authenticateJWT, authorizeRoles('admin', 'auditor'), (req, res) => {
    ...
    const validation = validateLicenseKey(resolveLicenseKey(req.headers['x-poseidon-license']), process.env.POSEIDON_LICENSE_SECRET, revokedHashes);
    res.status(200).json({
        mode,
...
router.post('/tools/validate', authenticateJWT, authorizeRoles('admin'), (req, res) => {
    ...
    const validation = validateLicenseKey(licenseKey, process.env.POSEIDON_LICENSE_SECRET, revokedHashes, nowMs);
    logger.info({
