...
function signToken(role) {
    return jwt.sign({ id: '1', role }, process.env.JWT_SECRET, { expiresIn: '1h' });
}
function buildApp() {
    const app = express();
...
describe('license route tools', () => {
    ...
    beforeEach(() => {
        ...
        revocationStorePath = path.join(os.tmpdir(), `poseidon-license-revocations-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}.json`);
    ...
    it('rotates a valid key and revokes previous by default', async () => {
        ...
        expect(rotateResponse.body.newKeyHash).toBe(hashLicenseKey(normalizeLicenseKey(rotateResponse.body.licenseKey)));
