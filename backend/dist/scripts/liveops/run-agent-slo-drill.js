...
async function main() {
    ...
    const policyPath = path.resolve(args.policyPath ?? path.join(repoRoot, 'governance/policies/agent-slo-policy.json'));
    const reportPath = path.resolve(args.reportDir ?? path.join(backendRoot, 'tmp/liveops-artifacts'), `agent-03-slo-${timestamp}.json`);
    const telemetryPath = path.resolve(args.telemetryDir ?? path.join(backendRoot, 'tmp/agent-slo', runId), 'agent-slo-telemetry.ndjson');
