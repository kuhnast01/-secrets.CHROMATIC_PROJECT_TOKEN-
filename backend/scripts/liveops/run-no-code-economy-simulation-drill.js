...
async function main() {
    ...
    const policyPath = node_path_1.default.resolve(args.policyPath ?? node_path_1.default.join(repoRoot, 'governance/policies/no-code-economy-simulation-policy.json'));
    const reportPath = node_path_1.default.resolve(args.reportDir ?? node_path_1.default.join(backendRoot, 'tmp/liveops-artifacts'), `nc-02-economy-simulation-${timestamp}.json`);
    const simulationPath = node_path_1.default.resolve(args.workDir ?? node_path_1.default.join(backendRoot, 'tmp/liveops-simulation', runId), 'economy-simulation.json');
    await promises_1.default.mkdir(node_path_1.default.dirname(reportPath), { recursive: true });
    await promises_1.default.mkdir(node_path_1.default.dirname(simulationPath), { recursive: true });
