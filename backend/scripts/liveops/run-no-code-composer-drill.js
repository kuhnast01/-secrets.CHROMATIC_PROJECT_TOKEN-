...
async function main() {
    ...
    const reportPath = node_path_1.default.resolve(args.reportDir ?? node_path_1.default.join(backendRoot, 'tmp/liveops-artifacts'), `nc-01-no-code-composer-${timestamp}.json`);
    const stagePath = node_path_1.default.resolve(args.stageDir ?? node_path_1.default.join(backendRoot, 'tmp/liveops-staging/no-code'), `nc-01-event-${timestamp}.json`);
    const inputs = {
    ...
    await runStep('no-code-composer:validate-event', steps, async () => {
        const parsed = validate_1.eventSchema.safeParse(draftedEvent);
        if (!parsed.success) {
