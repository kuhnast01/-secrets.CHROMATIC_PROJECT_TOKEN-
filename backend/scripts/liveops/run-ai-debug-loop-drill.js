...
async function writeSeedArtifacts(logArtifactPath, testArtifactPath) {
    ...
    await promises_1.default.mkdir(node_path_1.default.dirname(logArtifactPath), { recursive: true });
    await promises_1.default.mkdir(node_path_1.default.dirname(testArtifactPath), { recursive: true });
    await promises_1.default.writeFile(logArtifactPath, `${logLines.join('\n')}\n`, 'utf8');
    await promises_1.default.writeFile(testArtifactPath, JSON.stringify(testArtifact, null, 2), 'utf8');
}
...
async function main() {
    ...
    const policyPath = node_path_1.default.resolve(args.policyPath ?? node_path_1.default.join(repoRoot, 'governance/policies/ai-debug-loop-policy.json'));
    const reportPath = node_path_1.default.resolve(args.reportDir ?? node_path_1.default.join(backendRoot, 'tmp/liveops-artifacts'), `ai-02-debug-loop-${timestamp}.json`);
    const workDir = node_path_1.default.resolve(args.workDir ?? node_path_1.default.join(backendRoot, 'tmp/ai-debug-loop', runId));
    const logArtifactPath = node_path_1.default.join(workDir, 'debug.log');
    const testArtifactPath = node_path_1.default.join(workDir, 'test-results.json');
    const proposalPath = node_path_1.default.join(workDir, 'remediation-proposals.json');
    ...
    const logText = await promises_1.default.readFile(logArtifactPath, 'utf8');
    const testArtifact = JSON.parse(await promises_1.default.readFile(testArtifactPath, 'utf8'));
    ...
    await promises_1.default.writeFile(proposalPath, JSON.stringify({ schemaVersion: 1, runId, proposals }, null, 2), 'utf8');
