...
ipcMain.handle('poseidon:brain', async (_event, message) => {
    ...
    if (/\b(runbook|incident)\b/.test(lower)) {
        ...
        if (/\b(status|show)\b/.test(lower) && idMatch) {
            ...
            const pending = status.run.steps.filter((s) => s.status === 'pending').length;
            const failed = status.run.steps.filter((s) => s.status === 'failed').length;
