...
async function main() {
    ...
    const policyPath = path.resolve(args.policyPath ?? path.join(repoRoot, 'governance/policies/no-code-rollout-safety-policy.json'));
    const reportPath = path.resolve(args.reportDir ?? path.join(backendRoot, 'tmp/liveops-artifacts'), `nc-03-rollout-safety-${timestamp}.json`);
    const rolloutPlanPath = path.resolve(args.workDir ?? path.join(backendRoot, 'tmp/liveops-rollout-safety', runId), 'rollout-plans.json');
    ...
    await fs.writeFile(rolloutPlanPath, JSON.stringify({
        schemaVersion: 1,
        runId,
