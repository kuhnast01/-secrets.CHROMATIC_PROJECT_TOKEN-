...
async function resolveToken(baseUrl, args) {
    ...
    const response = await axios.post(`${baseUrl}/auth/login`, { username, password }, { timeout: 10000 });
...
async function main() {
    ...
    for (let index = 0; index < keys.length; index += 1) {
        ...
        const response = await axios.post(`${baseUrl}/license/tools/validate`, { licenseKey }, { headers, timeout: 10000 });
