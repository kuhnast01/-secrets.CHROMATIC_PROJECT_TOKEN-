...
class BuildingSystem {
    ...
    getBaseGeneration(type, level) {
        switch (type) {
            case Building_1.BuildingType.Generator:
                return { [Resource_1.ResourceType.Energy]: Math.round(10 * Math.pow(1.25, level - 1)) };
            case Building_1.BuildingType.Shipyard:
                return { [Resource_1.ResourceType.Alloy]: Math.round(5 * Math.pow(1.25, level - 1)) };
            case Building_1.BuildingType.ResearchLab:
                return { [Resource_1.ResourceType.Data]: Math.round(2 * Math.pow(1.25, level - 1)) };
            case Building_1.BuildingType.Storage:
                return { [Resource_1.ResourceType.Credits]: Math.round(8 * Math.pow(1.25, level - 1)) };
            default:
                return {};
        }
    }
