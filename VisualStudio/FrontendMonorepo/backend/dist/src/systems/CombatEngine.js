...
export class CombatEngine {
     ...
     */
    static simulateBattle(attackerFleet, defenderFleet, maxRounds = 20) {
        const log = [];
        let round = 1;
    ...
    static calcEffectiveHP(hp, def) {
        return hp * (1 + def / 100);
    }
    static calcEffectiveDamage(atk, crit, critDmg) {
        return atk * (1 + crit * (critDmg - 1));
    }
    static calcBaseDamage(atk, def) {
        return atk * (100 / (100 + def));
    }
    static calcCritDamage(dmg, critDmg) {
        return dmg * critDmg;
    }
    static calcAbilityDamage(base, ap) {
        return base * (1 + ap);
    }
    static minDamageRule(atk, dmg) {
        return Math.max(dmg, atk * 0.05);
    }
    static calcShipPower(ship) {
        const s = ship.stats;
        return s.atk + s.def + (s.hp / 10) + (s.spd * 2);
    }
    static calcCommanderPower(commander, baseStats, abilityPower, rarityMultiplier) {
        return (baseStats * rarityMultiplier) + abilityPower;
    }
    static calcFleetPower(fleet, rarityMultiplier = 1, abilityPower = 0) {
     ...
     */
    static determineTurnOrder(ships) {
     ...
     */
    static simulateRound(attackerFleet, defenderFleet) {
        const log = [];
        // --- Buff/Debuff Integration ---
        // Attach a BuffDebuffEngine to each fleet if not present
        if (!attackerFleet._buffEngine)
            attackerFleet._buffEngine = new (require('./BuffDebuffEngine').BuffDebuffEngine)();
        if (!defenderFleet._buffEngine)
            defenderFleet._buffEngine = new (require('./BuffDebuffEngine').BuffDebuffEngine)();
        const attackerBuffs = attackerFleet._buffEngine;
        const defenderBuffs = defenderFleet._buffEngine;
        ...
        function applyCommanderPassive(fleet, label) {
        ...
        function logBuffs(fleet, label, buffs) {
            if (buffs.buffs.length > 0)
                log.push(`${label} buffs: ` + buffs.buffs.map((b) => `${b.type} x${b.stacks}`).join(', '));
            if (buffs.debuffs.length > 0)
                log.push(`${label} debuffs: ` + buffs.debuffs.map((d) => `${d.type} x${d.stacks}`).join(', '));
        }
