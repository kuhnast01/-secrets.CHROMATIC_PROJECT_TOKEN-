...
exports.AgentCollaborationManager = void 0;
class AgentCollaborationManager {
    constructor(agents, contexts) {
        this.agents = agents;
        this.contexts = contexts;
    }
    // Sequential workflow: each agent executes a step in order
    async runSequentialWorkflow(steps) {
        const results = [];
    ...
    async runParallelWorkflow(agentSteps) {
        return Promise.all(agentSteps.map((steps, idx) => {
    ...
    async runConcurrentConflictWorkflow(actions, policy = 'priority') {
        ...
        const resolutions = [];
        const winnerResults = await Promise.all([...grouped.entries()].map(async ([resource, contenders]) => {
...
exports.AgentCollaborationManager = AgentCollaborationManager;
