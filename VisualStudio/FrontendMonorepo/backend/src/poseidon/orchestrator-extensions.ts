...
export async function enhancedApprovalCallback(step: OrchestratorStep, user: string): Promise<boolean> {
  ...
  if (step.description.toLowerCase().includes('danger')) return false;
  return true;
}

// Example: Safety guardrail wrapper for orchestrator plans
export async function safeExecutePlan(
