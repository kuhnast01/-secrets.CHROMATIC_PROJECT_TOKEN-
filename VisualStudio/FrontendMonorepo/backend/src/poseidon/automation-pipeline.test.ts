...
import { AutomationPipeline, AutomationTask } from './automation-pipeline';
...
describe('AutomationPipeline', () => {
  ...
  it('should prepare PR for lint-fix', async () => {
    const task: AutomationTask = { type: 'lint-fix', payload: {}, triggeredBy: 'cron', safeMode: true };
  ...
  it('should prepare PR for dependency-bump', async () => {
    const task: AutomationTask = { type: 'dependency-bump', payload: {}, triggeredBy: 'cron', safeMode: true };
    const result = await pipeline.runTask(task);
    expect(result.prPrepared).toBe(true);
  ...
  it('should validate content', async () => {
    const task: AutomationTask = { type: 'content-validation', payload: {}, triggeredBy: 'event', safeMode: true };
    const result = await pipeline.runTask(task);
  ...
  it('should stage LiveOps publish PR', async () => {
    const task: AutomationTask = { type: 'liveops-publish', payload: {}, triggeredBy: 'commit', safeMode: true };
    const result = await pipeline.runTask(task);
    expect(result.prPrepared).toBe(true);
  ...
  it('should trigger automation based on event', () => {
    expect(AutomationPipeline.shouldTrigger('lint-fix', 'cron')).toBe(true);
    expect(AutomationPipeline.shouldTrigger('liveops-publish', 'commit')).toBe(true);
    expect(AutomationPipeline.shouldTrigger('dependency-bump', 'event')).toBe(false);
  });
});
