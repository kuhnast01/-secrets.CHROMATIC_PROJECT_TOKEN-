...
class ToolError extends Error {
  constructor(
    message: string,
    public readonly errorType: 'usage' | 'auth' | 'api',
...
function usageError(message: string): ToolError {
  return new ToolError(message, 'usage', 2);
}

function authError(message: string): ToolError {
...
const DEFAULT_LABELS: LabelSpec[] = [
...
function parseRepoContext(raw?: string): RepoContext {
...
function printUsage(): void {
  console.log(
    'Usage: pnpm run stress:labels:bootstrap -- [owner/repo] [--repo owner/repo] [--dry-run] [--list-defaults] [--json]'
  );
  console.log('');
  console.log('Flags:');
...
function printUsageJson(): void {
  emitJson({
    ok: true,
    mode: 'help',
    message:
      'Usage: stress:labels:bootstrap [owner/repo] [--repo owner/repo] [--dry-run] [--list-defaults] [--json]',
  });
}

function printDefaultLabels(): void {
...
function argvHasJsonFlag(argv: string[]): boolean {
...
async function githubRequest<T>(token: string, path: string, method = 'GET', body?: unknown): Promise<T> {
...
async function main(): Promise<void> {
  ...
  const existingLabels = await githubRequest<Array<{ name: string }>>(
    token,
