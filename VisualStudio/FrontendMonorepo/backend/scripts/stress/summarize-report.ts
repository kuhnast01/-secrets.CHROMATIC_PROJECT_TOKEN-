...
type RegressionConfig = {
  ...
  maxRequestDropRatio: number;
  failOnWarning: boolean;
};

type RegressionWarning = {
  path: string;
  metric: 'p95' | 'errorRate' | 'requestDrop';
  current: number;
  baseline: number;
  threshold: number;
  delta: number;
...
function loadReport(path: string): StressReport {
  if (!existsSync(path)) {
    throw new Error(`Stress report not found: ${path}`);
  }

  return JSON.parse(readFileSync(path, 'utf8')) as StressReport;
}

function maybeLoadBaseline(path: string | undefined): StressReport | undefined {
...
function getRegressionConfig(): RegressionConfig {
...
function getRegressionWarnings(
  report: StressReport,
  baseline: StressReport | undefined,
...
function buildSummary(
  ...
  for (const result of report.results) {
    lines.push(
      `| ${result.path} | ${result.total} | ${result.p95} | ${formatPercent(result.errorRate)} | ${
  ...
  if (baselineComparable && baseline) {
    ...
    if (warnings.length === 0) {
      ...
      for (const warning of warnings) {
        ...
        const deltaDisplay =
          warning.metric === 'errorRate' || warning.metric === 'requestDrop'
        ...
        lines.push(
          `| ${warning.path} | ${warning.metric} | ${baselineDisplay} | ${currentDisplay} | ${deltaDisplay} | ${thresholdDisplay} |`
        );
      }
    }
