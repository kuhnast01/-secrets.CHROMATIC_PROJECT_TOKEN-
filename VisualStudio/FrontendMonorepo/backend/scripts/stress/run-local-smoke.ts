...
type StressProfile = 'smoke' | 'spike' | 'soak';

const profileArg = (process.argv[2] || 'smoke') as StressProfile;
const durationArg = process.argv[3];
const allowedProfiles: StressProfile[] = ['smoke', 'spike', 'soak'];

const backendBaseUrl = process.env.BACKEND_BASE_URL || 'http://127.0.0.1:4000';
const healthPath = '/healthz';
const startupTimeoutMs = Number(process.env.STRESS_STARTUP_TIMEOUT_MS || 90_000);

function sleep(ms: number): Promise<void> {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

function commandForPnpm(): { command: string; args: string[] } {
...
function terminateProcessTree(childPid: number): Promise<void> {
  return new Promise((resolve) => {
