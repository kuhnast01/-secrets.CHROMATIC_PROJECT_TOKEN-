...
function checkHealth(url) {
    return new Promise((resolve) => {
        const req = (0, node_http_1.request)(`${url}${healthPath}`, { method: 'GET', timeout: 4000 }, (res) => {
...
function terminateProcessTree(childPid) {
    return new Promise((resolve) => {
        if (process.platform === 'win32') {
            const killer = (0, node_child_process_1.spawn)('taskkill', ['/PID', String(childPid), '/T', '/F'], {
                stdio: 'ignore'
            });
...
async function main() {
    ...
    (0, node_fs_1.mkdirSync)(stressArtifactsDir, { recursive: true });
    const { command, args } = commandForPnpm();
    const backendProcess = (0, node_child_process_1.spawn)(`${command} ${[...args, 'run', 'dev'].join(' ')}`, {
        stdio: 'inherit',
        shell: true,
        env: process.env
    });
    ...
    try {
        ...
        await new Promise((resolve, reject) => {
            const durationOverride = durationArg && Number.isFinite(Number(durationArg)) && Number(durationArg) > 0
                ...
                : [];
            const stress = (0, node_child_process_1.spawn)(`${command} ${[
                ...args,
                'exec',
                'tsx',
