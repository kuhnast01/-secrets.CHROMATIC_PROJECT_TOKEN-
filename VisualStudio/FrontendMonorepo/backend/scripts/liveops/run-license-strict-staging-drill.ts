...
type DrillArtifact = {
  ...
  smoke: {
    missingLicenseCode: string | null;
    invalidFormatCode: string | null;
    revokedLicenseCode: string | null;
    validLicenseStatus: number;
  };
  endurance: {
    requestCount: number;
    successCount: number;
...
async function readPolicy(policyPath: string): Promise<StrictStagingPolicy> {
  const raw = await fs.readFile(policyPath, 'utf8');
  return JSON.parse(raw) as StrictStagingPolicy;
}
...
async function runEndurance({ app, validLicense, requestCount }: { app: ReturnType<typeof buildApp>; validLicense: string; requestCount: number; }) {
  let successCount = 0;
  let failureCount = 0;
...
async function main(): Promise<void> {
  ...
  const policyPath = path.resolve(
    args.policyPath ?? path.join(repoRoot, 'governance/policies/license-strict-staging-rollout-policy.json'),
  );
  const reportPath = path.resolve(
    args.reportDir ?? path.join(backendRoot, 'tmp/liveops-artifacts'),
    `lic-02-license-strict-staging-${timestamp}.json`,
  );
  const workPath = path.resolve(
    args.workDir ?? path.join(backendRoot, 'tmp/licensing-strict-staging', runId),
  );
  ...
  const checks: DrillArtifact['checks'] = [
    ...
    },
    {
      ...
      status:
        endurance.successRate >= policy.endurance.minSuccessRate &&
