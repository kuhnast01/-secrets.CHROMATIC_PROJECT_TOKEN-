...
type ProposalPriority = 'P0' | 'P1' | 'P2';

type DebugLoopPolicy = {
  schemaVersion: number;
  description: string;
  minimumProposalCount: number;
  requireCommandSuggestion: boolean;
  requireEvidenceReference: boolean;
  priorityOrder: ProposalPriority[];
  ownerByCategory: Record<string, string>;
};
...
type RemediationProposal = {
  ...
  category: 'test-failure' | 'regression' | 'latency' | 'dependency';
  owner: string;
  summary: string;
...
async function readPolicy(policyPath: string): Promise<DebugLoopPolicy> {
  const raw = await fs.readFile(policyPath, 'utf8');
  return JSON.parse(raw) as DebugLoopPolicy;
}

async function writeSeedArtifacts(logArtifactPath: string, testArtifactPath: string): Promise<void> {
  const logLines = [
...
function parseLogErrors(logText: string): string[] {
  return logText
    .split(/\r?\n/)
    .map((line) => line.trim())
...
function validateProposals(policy: DebugLoopPolicy, proposals: RemediationProposal[]): Array<{ name: string; status: DrillStatus; details: string }> {
  const checks: Array<{ name: string; status: DrillStatus; details: string }> = [];
...
async function main(): Promise<void> {
  ...
  const policyPath = path.resolve(
    args.policyPath ?? path.join(repoRoot, 'governance/policies/ai-debug-loop-policy.json'),
  );

  const reportPath = path.resolve(
    args.reportDir ?? path.join(backendRoot, 'tmp/liveops-artifacts'),
    `ai-02-debug-loop-${timestamp}.json`,
  );
  ...
  const testArtifact = JSON.parse(await fs.readFile(testArtifactPath, 'utf8')) as TestArtifact;
