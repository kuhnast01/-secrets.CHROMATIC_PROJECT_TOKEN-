...
type AgentSample = {
  agent: 'engineering-agent' | 'qa-agent' | 'sre-agent';
  operation: string;
  latencyMs: number;
  success: boolean;
...
type DrillArtifact = {
  ...
  thresholds: {
    ...
    latencyAverageMsMax: number;
    successRateMin: number;
    rollbackRateMin: number;
    rollbackDurationMsMax: number;
  };
  checks: Array<{
    name: string;
    status: DrillStatus;
...
async function readPolicy(policyPath: string): Promise<SloPolicy> {
  const raw = await fs.readFile(policyPath, 'utf8');
  return JSON.parse(raw) as SloPolicy;
}

function buildSamples(): AgentSample[] {
  const now = new Date();
  const operations: Array<{
    agent: AgentSample['agent'];
    operation: string;
    latencyMs: number;
    success: boolean;
    rollbackTriggered?: boolean;
...
function computeMetrics(samples: AgentSample[]): SloMetrics {
  ...
  return {
    ...
    latencyAverageMs:
      totalOperations === 0 ? 0 : latencies.reduce((sum, value) => sum + value, 0) / totalOperations,
...
async function main(): Promise<void> {
  ...
  const policyPath = path.resolve(
    args.policyPath ?? path.join(repoRoot, 'governance/policies/agent-slo-policy.json'),
  );
  const reportPath = path.resolve(
    args.reportDir ?? path.join(backendRoot, 'tmp/liveops-artifacts'),
    `agent-03-slo-${timestamp}.json`,
  );
  const telemetryPath = path.resolve(
    args.telemetryDir ?? path.join(backendRoot, 'tmp/agent-slo', runId),
    'agent-slo-telemetry.ndjson',
  );
  ...
  await fs.writeFile(telemetryPath, samples.map((sample) => JSON.stringify(sample)).join('\n') + '\n', 'utf8');
