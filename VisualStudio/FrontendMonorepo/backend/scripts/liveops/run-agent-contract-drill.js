...
async function main() {
    ...
    const reportPath = node_path_1.default.resolve(args.reportDir ?? node_path_1.default.join(backendRoot, 'tmp/liveops-artifacts'), `ai-01-agent-contract-${timestamp}.json`);
    const contractPath = node_path_1.default.resolve(args.contractDir ?? node_path_1.default.join(backendRoot, 'tmp/agent-contracts'), `agent-contracts-${timestamp}.json`);
    const steps = [];
    try {
        ...
        const sandboxRoot = node_path_1.default.resolve(backendRoot, 'tmp/agent-contract-deny-default');
        node_fs_1.default.mkdirSync(sandboxRoot, { recursive: true });
        const noWriteContext = {
            user: 'security-enforcer',
            sandboxRoot,
            allowWrites: false,
        };
        const guardedAgent = new agent_1.PoseidonAgent(noWriteContext);
        try {
