...
type EconomyPolicy = {
  ...
  description: string;
  riskThresholds: {
    maxEconomyDeltaPercent: number;
    maxWhaleAdvantageIndex: number;
    maxInflationRiskScore: number;
...
type SimulationSignal = {
  name: 'economy_delta_percent' | 'whale_advantage_index' | 'inflation_risk_score' | 'payout_per_user';
  value: number;
  threshold: number;
  passed: boolean;
};
...
type DrillArtifact = {
  ...
  validScenario: ScenarioResult;
  invalidScenario: ScenarioResult;
};
...
async function readPolicy(policyPath: string): Promise<EconomyPolicy> {
  const raw = await fs.readFile(policyPath, 'utf8');
  return JSON.parse(raw) as EconomyPolicy;
}
...
function evaluateScenario(
  ...
  policy: EconomyPolicy,
  values: {
    economyDeltaPercent: number;
    whaleAdvantageIndex: number;
  ...
  const publishDecision: 'allow' | 'block' = policy.blockPublishOnViolation
    ? hasViolations
      ? 'block'
      : 'allow'
    : 'allow';
...
async function main(): Promise<void> {
  ...
  const policyPath = path.resolve(
    args.policyPath ?? path.join(repoRoot, 'governance/policies/no-code-economy-simulation-policy.json'),
  );
  const reportPath = path.resolve(
    args.reportDir ?? path.join(backendRoot, 'tmp/liveops-artifacts'),
    `nc-02-economy-simulation-${timestamp}.json`,
  );
  const simulationPath = path.resolve(
    args.workDir ?? path.join(backendRoot, 'tmp/liveops-simulation', runId),
    'economy-simulation.json',
  );
