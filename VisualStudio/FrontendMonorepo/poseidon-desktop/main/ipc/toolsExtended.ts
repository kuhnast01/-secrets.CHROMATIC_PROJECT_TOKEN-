...
type TestRunnerName = 'jest' | 'cypress' | 'detox';
...
interface ObservabilityServiceLog {
  name: ServiceName;
  lines: string[];
}

type RunbookName = 'backend-outage';
...
interface IncidentRun {
  ...
  createdAt: string;
  updatedAt: string;
  steps: IncidentRunStep[];
  timeline: string[];
}

const workspaceRoot = process.cwd();
const isWindows = process.platform === 'win32';
const managedProcesses = new Map<ServiceName, ChildProcessWithoutNullStreams>();
const managedLogs = new Map<ServiceName, string[]>();
const MAX_LOG_LINES = 500;

const serviceDefinitions: Record<ServiceName, ServiceDefinition> = {
  backend: {
    name: 'backend',
    port: 4000,
    command: 'pnpm',
...
const managedTestRunners: Record<TestRunnerName, string[]> = {
  jest: ['pnpm', 'test'],
  cypress: ['pnpm', 'run', 'e2e:cypress:endurance:quick'],
  detox: ['pnpm', 'run', 'e2e:detox:endurance:quick'],
};

const incidentRunbookDefinitions: Record<RunbookName, RunbookDefinition> = {
  'backend-outage': {
    name: 'backend-outage',
...
const incidentRuns = new Map<string, IncidentRun>();
...
function appendManagedLog(service: ServiceName, chunk: string): void {
...
async function waitForPortOpen(port: number, timeoutMs = 20000): Promise<boolean> {
...
function getManagedProcess(name: ServiceName): ChildProcessWithoutNullStreams | null {
...
async function stopManagedService(name: ServiceName): Promise<{ status: string; message?: string }> {
...
async function startManagedService(name: ServiceName): Promise<{
  status: string;
  message?: string;
...
function parseServiceName(input: string): ServiceName | null {
...
function parseRunnerName(input: string): TestRunnerName | null {
...
function createIncidentRun(name: RunbookName): IncidentRun {
...
function getIncidentRun(runId: string): IncidentRun | null {
  return incidentRuns.get(runId) ?? null;
}

async function executeIncidentStep(run: IncidentRun, stepId: string): Promise<IncidentRunStep> {
...
ipcMain.handle('tools:runCommand', async (_event: IpcMainInvokeEvent, command: string) => {
  ...
  return await new Promise((resolve) => {
    exec(trimmed, { timeout: 15000 }, (err: Error | null, stdout: string, stderr: string) => {
      if (err) return resolve({ status: 'error', error: stderr || err.message });
...
ipcMain.handle('tools:serviceStatus', async (_event: IpcMainInvokeEvent, serviceName?: string) => {
  if (!serviceName) {
...
ipcMain.handle('tools:serviceStart', async (_event: IpcMainInvokeEvent, serviceName: string) => {
  const parsed = parseServiceName(serviceName);
  if (!parsed) {
    return { status: 'error', error: `Unknown service: ${serviceName}` };
  }
...
ipcMain.handle('tools:serviceStop', async (_event: IpcMainInvokeEvent, serviceName: string) => {
  const parsed = parseServiceName(serviceName);
  if (!parsed) {
    return { status: 'error', error: `Unknown service: ${serviceName}` };
  }
...
ipcMain.handle('tools:serviceRestart', async (_event: IpcMainInvokeEvent, serviceName: string) => {
  const parsed = parseServiceName(serviceName);
  if (!parsed) {
    return { status: 'error', error: `Unknown service: ${serviceName}` };
  }
...
ipcMain.handle('tools:serviceLogs', async (_event: IpcMainInvokeEvent, serviceName: string, lines = 80) => {
  const parsed = parseServiceName(serviceName);
  if (!parsed) {
    return { status: 'error', error: `Unknown service: ${serviceName}` };
  }
...
ipcMain.handle('tools:readEnv', async (_event: IpcMainInvokeEvent, filePath = '.env') => {
  try {
...
ipcMain.handle('tools:searchLogs', async (_event: IpcMainInvokeEvent, filePath: string, pattern = 'error|fail|warn') => {
  try {
    const abs = path.isAbsolute(filePath) ? filePath : path.join(process.cwd(), filePath);
    const content = fs.readFileSync(abs, 'utf8');
...
ipcMain.handle('tools:getGitStatus', async () => {
  return await new Promise((resolve) => {
    exec('git status --porcelain -b', { timeout: 10000 }, (err: Error | null, stdout: string, stderr: string) => {
      if (err) return resolve({ status: 'error', error: stderr || err.message });
      resolve({ status: 'ok', output: stdout });
    });
  });
...
ipcMain.handle('tools:runPlugin', async (_event: IpcMainInvokeEvent, pluginName: string, ...args: unknown[]) => {
  try {
    ...
    if (typeof plugin !== 'function') throw new Error('Plugin must export default function');
...
ipcMain.handle('tools:trackMetric', async (_event: IpcMainInvokeEvent, metric: string, value: unknown) => {
  // For now, just log
...
ipcMain.handle('tools:runTests', async (_event: IpcMainInvokeEvent, runner = 'jest') => {
  ...
  return await new Promise((resolve) => {
    exec(shellCommand, { cwd: workspaceRoot, timeout: 180000 }, (err: Error | null, stdout: string, stderr: string) => {
      if (err) return resolve({ status: 'error', error: stderr || err.message, runner: parsed });
      resolve({ status: 'ok', output: stdout });
    });
  });
});
...
ipcMain.handle('tools:observabilitySnapshot', async () => {
  ...
  const recentLogs: ObservabilityServiceLog[] = (['backend', 'admin-panel', 'web'] as ServiceName[]).map((name) => {
...
ipcMain.handle('tools:incidentRunbookStart', async (_event: IpcMainInvokeEvent, name: RunbookName) => {
...
ipcMain.handle('tools:incidentRunbookStatus', async (_event: IpcMainInvokeEvent, runId: string) => {
...
ipcMain.handle('tools:incidentRunbookExecuteStep', async (_event: IpcMainInvokeEvent, runId: string, stepId: string) => {
  const run = getIncidentRun(runId);
  if (!run) {
    return { status: 'error', error: `Unknown run ID: ${runId}` };
  }
...
ipcMain.handle('tools:incidentRunbookExecuteNext', async (_event: IpcMainInvokeEvent, runId: string) => {
  const run = getIncidentRun(runId);
  if (!run) {
    return { status: 'error', error: `Unknown run ID: ${runId}` };
  }
