...
ipcMain.handle('tools:checkPort', async (_event, port) => {
...
ipcMain.handle('tools:readFile', async (_event, filePath) => {
    try {
...
ipcMain.handle('tools:tailLog', async (_event, filePath, lines = 40) => {
    try {
        const abs = path.isAbsolute(filePath) ? filePath : path.join(process.cwd(), filePath);
        const content = fs.readFileSync(abs, 'utf8');
